...
function calculator() {
  ...
  var calcTotal = function calcTotal() {
    ...
    if (sex === 'female') {
      return result.textContent = Math.round((447.6 + 9.2 * weight + 3.1 * height - 4.3 * age) * ratio);
    } else {
      return result.textContent = Math.round((88.36 + 13.4 * weight + 4.8 * height - 5.7 * age) * ratio);
    }
  };
...
/* harmony default export */ const __WEBPACK_DEFAULT_EXPORT__ = (cards); // getResource('http://localhost:3000/menu')
...
function forms(formSelector, modalTimerId) {
  ...
  var message = {
    loading: 'spinner.svg',
    success: 'Thanks for communication',
  ...
  function bindPostData(form) {
    form.addEventListener('submit', function (e) {
      ...
      form.insertAdjacentElement('afterend', statusMessage); // when you use XMLHttpRequest you don't have to config headers
...
function slider(_ref) {
  ...
  var stringToIntLength = function stringToIntLength(width) {
    return +width.replace(/\D/g, '');
  },
      createDotsList = function createDotsList(indicators) {
    indicators.forEach(function (dot) {
      return dot.style.opacity = '.5';
    });
    indicators[slideIndex - 1].style.opacity = 1;
  },
      createSliderPrevButton = function createSliderPrevButton(button) {
    button.addEventListener('click', function () {
      ...
      slideField.style.transform = "translateX(-".concat(offset, "px)");
      ...
      createSliderNextButton = function createSliderNextButton() {
  ...
  createSliderNextButton(next); // PREVIOUS-BUTTON-SLIDER
...
function timer(id, deadline) {
  ...
  function setClock(selector, endtime) {
    ...
    function updateClock() {
      ...
      hours.innetHTML = getZero(t.hours);
      mins.innerHTML = getZero(t.minutes);
      secs.innerHTML = getZero(t.seconds);
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  (0,_modules_timer__WEBPACK_IMPORTED_MODULE_8__.default)('.timer', '2021-08-11');
